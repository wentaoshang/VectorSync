...
public:
  ...
  bool Insert(const ESN& esn) {
    if (esn.vi.first == 0 || esn.vi.second.empty()
        || esn.rn == 0 || esn.seq == 0 || esn.seq > 255)
      return false;
    ...
    if (!entry.leader_id.empty()) {
      ...
      if (entry.leader_id != esn.vi.second)
        return false;
    } else {
  ...
  SeqNumIntervalSet CheckForMissingData(const ESN& ldi, const ViewID& vid,
    ...
    if (ldi.vi.first == 0 || ldi.rn == 0 || ldi.seq == 0)
      return {};

    // (vid, rn) must be greater than (ldi.vid, ldi.rn).
    if (vid.first < ldi.vi.first
        || (vid.first == ldi.vi.first && rn < ldi.rn))
      return {};

    // If vid and ldi have the same view num, the leader id must also be
    // the same.
    if (vid.first == ldi.vi.first && vid.second != ldi.vi.second)
      return {};
    ...
    if (!entry.leader_id.empty()) {
      if (entry.leader_id != ldi.vi.second)
        return {};
    } else {
    ...
    if (entry.last_seq_num != 0) {
      ...
      if (entry.last_seq_num != uint8_t(ldi.seq))
        return {};
      if (entry.next_rn != rn)
        return {};
      if (entry.next_vi != vid)
        return {};
    } else {
  ...
  friend bool operator==(const ReceiveWindow& l,
                         const ReceiveWindow& r) {
    return l.state_ == r.state_;
  }

  friend std::ostream& operator<<(std::ostream& os,
                                  const ReceiveWindow& rw) {
    os << "ReceiveWindow{";
    for (const auto& p : rw.state_) {
      os << "[vi=(" << p.first.first << "," << p.second.leader_id << "),rn="
         << p.first.second << ",win=" << p.second.win << "]";
    }
    os << "}";
    return os;
  }
